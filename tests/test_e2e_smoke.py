...
def test_pipeline_exposes_expanded_run_id(tmp_path: Path, monkeypatch) -> None:
        ...
        yaml.safe_dump(
            {
                "name": "integrity",
                "run_id": "${QUANT_RUN_ID}",
                "steps": [{"name": "show", "cmd": "echo {QUANT_PIPELINE_RUN_ID}"}],
            }
        ),
        encoding="utf-8",
    )
    ...
    result = run_pipeline(pipe_cfg, dry_run=True)
    assert result.steps[0].command == "echo research_42"
